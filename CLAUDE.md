...
├── install.sh                        # インストーラ（symlink ではなくコピー）
├── LICENSE                           # MIT
...
1. `.claude/skills/cmux-team/templates/<role-name>.md` を作成
2. `{{VARIABLE}}` プレースホルダーを使用（下記参照）
3. Conductor が spawn 時にテンプレート変数を置換し `.team/prompts/` に書き出す

### install.sh への反映

新しいスキル・コマンドを追加した場合、`install.sh` のコピー対象に含まれているか確認する。現在の実装では `.claude/commands/*.md` と `.claude/skills/` 配下を一括コピーするため、通常は変更不要。
