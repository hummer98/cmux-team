...
Claude: タスクを作成しました。Manager が拾います。
  → Manager がタスクを検出 → Conductor を起動
  → Conductor が Agent を隣のペインに起動
  → 各エージェントの作業がリアルタイムで見える

あなた: 状況は？
Claude: （Manager 画面・cmux tree・タスクファイルを確認して報告）
        Conductor-1: 実装中（Agent 2/3 完了）

あなた: あと E2E テストも追加して
Claude: 追加のタスクを作成しました。次のタスクとして実行されます。
```
...
`specs/` と `tasks/` は git に含まれるので、設計判断の履歴が残ります。
...
**Manager の動作仕様**: Haiku モデル（`--model haiku`）で動作し、権限を制限（`--settings .team/settings.manager.json` — Bash/Read のみ、Edit/Write/Agent 禁止）。タスクがない場合はアイドル停止し、Master からの `[TASK_CREATED]` 通知で起床する。Conductor の起動は `.team/scripts/spawn-conductor.sh` に委譲。履歴は `.team/logs/manager.log` に記録。

### エージェントロール

| ロール | 担当 | 出力例 |
|--------|-----|--------|
| Manager | タスク監視（イベント駆動）、Conductor 起動、結果収集 | manager.log |
