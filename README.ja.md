...
Claude: .team/ を初期化しました。Conductor モードに入ります。
        実行計画:
        1. ✅ spec — 要件をブレスト
        2. ✅ research — 技術スタック調査
        3. ✅ design — アーキテクチャ設計
...
あなた: OK
Claude: まず要件定義から。どんな機能が必要ですか？
  ...壁打ちの後...
Claude: requirements.md を生成しました。リサーチフェーズに進みます。
        3つのリサーチャーを起動します。
  → 別ワークスペースにペインが3つ開き、それぞれが並列で調査
  → 調査の様子がリアルタイムで見える
  → 全員完了したら結果を統合して報告
Claude: リサーチ完了。結果はこちらです。設計に進みますか？

あなた: はい
Claude: アーキテクトを起動します。
  → 設計 → レビュー → フィードバック反映 を自動で実行
Claude: 設計が承認されました。タスク5件に分解。実装に進みますか？

あなた: はい
Claude: 実装エージェントを3体起動します。
  → 各エージェントがコードを書いている様子が見える
  ...
```

Conductor がフェーズを自律的に進行し、各フェーズの完了時にユーザーの承認を得ます。個別のコマンドで手動制御することもできます：

```
あなた: /team-research React vs Vue vs Svelte    （リサーチだけ手動実行）
あなた: /team-status                              （いつでも状態確認）
あなた: /team-disband                             （全エージェント停止）
```
...
| コマンド | やること | いつ使う |
|---------|---------|---------|
| `/team-init <ミッション>` | 初期化 + Conductor モード起動 | ミッション開始時 |
