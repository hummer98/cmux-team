...
**Manager の動作仕様**: Sonnet モデル（`--model sonnet`）で動作。タスクがない場合はアイドル停止し、Master からの `[TASK_CREATED]` 通知で起床する。Conductor の起動は `.team/scripts/spawn-conductor.sh` に委譲。履歴は `.team/logs/manager.log` に記録。
