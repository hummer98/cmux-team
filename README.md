...
graph LR
    You[あなた] --> Claude
    Claude --> BlackBox["[見えない箱]<br/>何を読んでる？<br/>何を書いてる？<br/>止まってない？"]
    BlackBox --> Result[結果だけ返ってくる]

    style BlackBox fill:#333,color:#fff,stroke:#666
```
...
```mermaid
graph TB
    subgraph cmux["cmux ターミナル"]
        direction LR
        Conductor["<b>Conductor</b><br/>(あなたと会話中)<br/><br/>&gt; 進捗は？"]
        Researcher["<b>Researcher</b><br/>(調査中...)<br/>ファイル読み中...<br/><br/>✶ Stewing..."]
        Architect["<b>Architect</b><br/>(設計書生成中)<br/><br/>✢ Writing..."]
        Reviewer["<b>Reviewer</b><br/>(レビューコメント<br/>作成中)<br/>✻ Reading.."]
    end
    Sidebar["サイドバー: 🔨 researcher: reading files | ✨ architect: done"]

    style cmux fill:#1a1a2e,color:#eee,stroke:#444
    style Conductor fill:#2d3436,color:#dfe6e9,stroke:#636e72
    style Researcher fill:#2d3436,color:#dfe6e9,stroke:#636e72
    style Architect fill:#2d3436,color:#dfe6e9,stroke:#636e72
    style Reviewer fill:#2d3436,color:#dfe6e9,stroke:#636e72
    style Sidebar fill:#0d1117,color:#8b949e,stroke:#30363d
```
...
```mermaid
graph TB
    You[あなた] <-->|自然言語| Conductor["Conductor<br/>(親 Claude)"]

    Conductor -->|cmux send| A[Agent A]
    Conductor -->|cmux send| B[Agent B]
    Conductor -->|cmux send| C[Agent C]

    A -->|ファイル書出| Output[".team/output/"]
    B -->|ファイル書出| Output
    C -->|ファイル書出| Output

    Output --> Read["Conductor が読み取り"]
    Read --> Integrate["結果を統合して報告"]
    Integrate --> You
```
