...
- [x] REQ-012: Agent 監視 — Conductor が `cmux-team await-agent` で done マーカーを fs.watch で待機（push 型）。生存は Manager の `spawnAgentPidWatcher` が `process.kill(pid, 0)` で 1 秒間隔チェック（T195）
