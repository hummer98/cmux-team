...
// --- T302: assign_skipped_terminal guard ---
// race window: scanTasks が assignTask(数秒〜十数秒) を走らせている間に
// delete-task / abort-task が task-state.json を terminal に書き換えると、
// 完了直後の `saveTaskState(status:'assigned')` で terminal が巻き戻る。
// ガード `__testApplyAssignCommit` は loadTaskState 結果を確認し terminal なら
// saveTaskState を skip して worktree cleanup + Conductor idle reset する。
describe("T302 assign_skipped_terminal guard", () => {
  // resetConductor は cmux.getPaneForSurface / listSiblingSurfaces / closeSurface を呼ぶため
  // 実 cmux が fake surface に対して undefined を返し broken 昇格してしまう。
  // ガードの意図（idle に戻す）を検証するためモックで surface 存在扱いにする。
  let paneSpy: ReturnType<typeof import("bun:test").spyOn>;
  let siblingsSpy: ReturnType<typeof import("bun:test").spyOn>;
  let closeSurfaceSpy: ReturnType<typeof import("bun:test").spyOn>;

  beforeEach(async () => {
    const cmux = await import("./cmux");
    const { spyOn } = await import("bun:test");
    paneSpy = spyOn(cmux, "getPaneForSurface").mockResolvedValue("pane:1");
    siblingsSpy = spyOn(cmux, "listSiblingSurfaces").mockResolvedValue([]);
    closeSurfaceSpy = spyOn(cmux, "closeSurface").mockResolvedValue(undefined as any);
  });

  afterEach(() => {
    paneSpy.mockRestore();
    siblingsSpy.mockRestore();
    closeSurfaceSpy.mockRestore();
  });

  test("task-state が deleted なら saveTaskState skip + worktree cleanup + conductor idle", async () => {
    ...
    const conductor: ConductorState = {
      ...
      taskRunId,
      worktreePath,
      outputDir: `.team/tasks/302-deleted-race/runs/${taskRunId}`,
    };
    state.conductors.set(conductor.surface, conductor);
    ...
    // Conductor は idle にリセット済み
    expect(conductor.status).toBe("idle");
    expect(conductor.taskId).toBeUndefined();
    expect(conductor.taskRunId).toBeUndefined();
    expect(conductor.worktreePath).toBeUndefined();

    // log 検証: assign_skipped_terminal + conductor_reset が記録される。
    // testDir は git init 済みでないため `git worktree remove` は失敗するが、
    // resetConductor 内で cleanup_failed ログに握りつぶされる（後続の reset に影響しない）。
    const logContent = await readFile(join(testDir, ".team/logs/manager.log"), "utf-8");
    expect(logContent).toContain("assign_skipped_terminal");
    expect(logContent).toContain("current_status=deleted");
    expect(logContent).toContain("conductor_reset");
  });
  ...
  test("task-state が aborted なら同じく skip", async () => {
    ...
    const conductor: ConductorState = {
      ...
      // worktreePath 未設定: resetConductor 内 existsSync ガードで cleanup スキップ
    };
    state.conductors.set(conductor.surface, conductor);
  ...
  test("task-state が closed なら skip（regression guard）", async () => {
    await createTask("306", "closed-race", { status: "ready" });

    const state = await createDaemon(testDir);
    state.mainBranch = "main";
    const conductor: ConductorState = {
      surface: "surface:fake-c306",
      startedAt: new Date().toISOString(),
      agents: [],
      status: "assigning",
      taskId: "306",
      taskRunId: "task-306-1700000004",
    };
    state.conductors.set(conductor.surface, conductor);

    const { saveTaskState, loadTaskState } = await import("./task");
    const ts0 = await loadTaskState(testDir);
    ts0["306"] = { status: "closed", closedAt: new Date().toISOString() };
    await saveTaskState(testDir, ts0);

    const { __testApplyAssignCommit } = await import("./daemon");
    const result = await __testApplyAssignCommit(state, "306", conductor);
    ...
    const tsAfter = await loadTaskState(testDir);
    expect(tsAfter["306"]?.status).toBe("closed");
    expect(tsAfter["306"]?.assignedAt).toBeUndefined();
    expect(conductor.status).toBe("idle");
  });
  ...
  test("task-state が ready（通常ケース）なら assigned に更新される", async () => {
    ...
    // Conductor はガード経由で reset されない（commit 経路）
    expect(conductor.status).toBe("assigning");
  });

  test("task-state 未作成エントリ（undefined status）は通常ケースとして assigned 書き込み", async () => {
    // race 極端ケース: delete-task が task-state entry ごと消した場合の defensive 挙動。
    // 現状 delete-task は status=deleted を残すため実環境では発生しないが、
    // undefined status ではガードを発動させず assigned を書く設計を検証する。
    const state = await createDaemon(testDir);
    state.mainBranch = "main";
    const conductor: ConductorState = {
